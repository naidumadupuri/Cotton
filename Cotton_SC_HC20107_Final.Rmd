...
```{r}
#select Varibales  for Head to Head analysis HC20107_Aatish

HC20107_Aatish <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Aatish")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Aatish", 
         Check_group = "Aatish", 
...
#select Varibales  for Head to Head analysis HC20107_Ajooba

HC20107_Ajooba <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Ajooba")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Ajooba", 
         Check_group = "Ajooba", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_MC5401

HC20107_MC5401 <- cottonData%>%
  filter(Hybrid %in% c("HC20107","MC5401")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_MC5401", 
         Check_group = "MC5401", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_Pinaka

HC20107_Pinaka <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Pinaka")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Pinaka", 
         Check_group = "Pinaka", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_Vega

HC20107_Vega <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Vega")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Vega", 
         Check_group = "Vega", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_RCH659

HC20107_RCH659 <- cottonData%>%
  filter(Hybrid %in% c("HC20107","RCH659")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_RCH659", 
         Check_group = "RCH659", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_Jadoo

HC20107_Jadoo <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Jadoo")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Jadoo", 
         Check_group = "Jadoo", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_Moksha

HC20107_Moksha <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Moksha")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Moksha", 
         Check_group = "Moksha", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_Jangee

HC20107_Jangee <- cottonData%>%
  filter(Hybrid %in% c("HC20107","Jangee")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_Jangee", 
         Check_group = "Jangee", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_MoneyMaker

HC20107_MoneyMaker <- cottonData%>%
  filter(Hybrid %in% c("HC20107","MoneyMaker")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_MoneyMaker", 
...
#select Varibales  for Head to Head analysis HC20107_ATM

HC20107_ATM <- cottonData%>%
  filter(Hybrid %in% c("HC20107","ATM")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_ATM", 
         Check_group = "ATM", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20107_RCHNeo

HC20107_RCHNeo <- cottonData%>%
  filter(Hybrid %in% c("HC20107","RCHNeo")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20107_RCHNeo", 
         Check_group = "RCHNeo", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
```{r}
All_combined <- bind_rows(HC20107_Aatish,
                          HC20107_Ajooba,
                          HC20107_Pinaka,
                          HC20107_MC5401,
                          HC20107_Vega,
...
"D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH1_Means_Wins_Yield_Superiority.xlsx")
...
superority <- HybridMeans %>% 
  ...
  mutate(Aatish_Superiority	=(HC20107_AatishNH-HC20107_AatishCC)/HC20107_AatishCC*100,
      Ajooba_Superiority =(HC20107_AjoobaNH-HC20107_AjoobaCC)/HC20107_AjoobaCC*100,
      MC5401_Superiority =(HC20107_MC5401NH-HC20107_MC5401CC)/HC20107_MC5401CC*100,
      Pinaka_Superiority =(HC20107_PinakaNH-HC20107_PinakaCC)/HC20107_PinakaCC*100,
      Vega_Superiority	= (HC20107_VegaNH-HC20107_VegaCC)/HC20107_VegaCC*100,
      ATM_Superiority	= (HC20107_ATMNH-HC20107_ATMCC)/HC20107_ATMCC*100,
      Jadoo_Superiority	= (HC20107_JadooNH-HC20107_JadooCC)/HC20107_JadooCC*100,
      Jangee_Superiority =(HC20107_JangeeNH-HC20107_JangeeCC)/HC20107_JangeeCC*100,
      Moksha_Superiority =(HC20107_MokshaNH-HC20107_MokshaCC)/HC20107_MokshaCC*100,
    MoneyMaker_Superiority = 
    (HC20107_MoneyMakerNH-HC20107_MoneyMakerCC)/HC20107_MoneyMakerCC*100,
    RCH659_Superiority =(HC20107_RCH659NH-HC20107_RCH659CC)/HC20107_RCH659CC*100,
    RCHNeo_Superiority	=(HC20107_RCHNeoNH-HC20107_RCHNeoCC)/HC20107_RCHNeoCC*100) %>%   mutate(across(is.numeric, ~ round(., 2))) %>% 
  mutate_all(funs(replace(., .== "Inf", NA))) %>%   
  mutate_all(funs(replace(., .== 0.00, NA))) %>%  
            rename("Attribute" = variable) %>% 
      select(Attribute, 
              HC20107_AatishNH, HC20107_AatishCC, Aatish_Superiority,
              HC20107_AjoobaNH, HC20107_AjoobaCC, Ajooba_Superiority,
              HC20107_PinakaNH, HC20107_PinakaCC, Pinaka_Superiority,
              HC20107_MC5401NH, HC20107_MC5401CC, MC5401_Superiority,
              HC20107_VegaNH,     HC20107_VegaCC,  Vega_Superiority, 
              HC20107_RCH659NH, HC20107_RCH659CC, RCH659_Superiority,
              HC20107_JadooNH,  HC20107_JadooCC, Jadoo_Superiority,
              HC20107_JangeeNH, HC20107_JangeeCC, Jangee_Superiority,
              HC20107_MokshaNH, HC20107_MokshaCC,Moksha_Superiority,
              HC20107_MoneyMakerNH, HC20107_MoneyMakerCC,MoneyMaker_Superiority,
              HC20107_ATMNH, HC20107_ATMCC,ATM_Superiority,
              HC20107_RCHNeoNH, HC20107_RCHNeoCC,RCHNeo_Superiority,
              everything()) %>% 
...
"D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH2_Superiority.xlsx")
...
```{r}
HC20107_all_Hybrid <- All_combined %>% 
    ungroup() %>% 
...
HC20107_all_State <- All_combined %>% 
    ungroup() %>% 
...
# Combine HC20107_all_State  and HC20107_all_Hybrid by rows

HC20107_State_Trials_Wins <- bind_rows(HC20107_all_State,
                                       HC20107_all_Hybrid)


write_xlsx(list(HC20107_All = HC20107_State_Trials_Wins),
           "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH3_State_Trials_Means_Wins_B.xlsx")
...
# 20A. Creating Summary tables for Excel Dash boards for HC20107
...
```{r}
HC20107_data <- All_combined %>% 
    select(Trial, State, Loc, Cropmanagmt,Sowingdate,Cultivation, Hybrid, Check_group, Yield) %>% 
      filter(Hybrid == "HC20107")
HC20107_checks <- All_combined %>% 
    filter(Hybrid != "HC20107") %>% 
    select(Trial, State, Yield)  

HC20107_checks_df <- cbind(HC20107_data,
                               HC20107_checks) %>% 
...
#Create  Datasets  for HEad to Head comparison of HC20107 with all Checks 


HC20107_Aatish_df <- HC20107_checks_df %>% 
  ...
  rename(HC20107 = NH_Yield,
         Aatish = CC_Yield)



HC20107_Ajooba_df <- HC20107_checks_df %>% 
...
HC20107_Pinaka_df <- HC20107_checks_df %>% 
...
HC20107_MC5401_df <- HC20107_checks_df %>% 
...
HC20107_Vega_df <- HC20107_checks_df %>% 
...
HC20107_RCH659_df <- HC20107_checks_df %>% 
...
HC20107_RCHNeo_df <- HC20107_checks_df %>% 
...
HC20107_Jadoo_df <- HC20107_checks_df %>% 
...
HC20107_Moksha_df <- HC20107_checks_df %>% 
...
HC20107_Jangee_df <- HC20107_checks_df %>% 
...
HC20107_MoneyMaker_df <- HC20107_checks_df %>% 
...
HC20107_ATM_df <- HC20107_checks_df %>% 
...
# A. Workbook for checkwise superority     
  ...
  wb <- createWorkbook()
    ...
    addWorksheet(wb, "HC20107_sup")
    
    writeData(wb, "HC20107_sup", HC20107_Aatish_df, 
              startRow = 1, startCol = 1)
    
     writeData(wb, "HC20107_sup", HC20107_Ajooba_df, 
              startRow = 47, startCol = 1)
     
    writeData(wb, "HC20107_sup", HC20107_Pinaka_df, 
              startRow = 63, startCol = 1)     
    
    writeData(wb, "HC20107_sup", HC20107_MC5401_df, 
              startRow = 92, startCol = 1)
    
    writeData(wb, "HC20107_sup", HC20107_Vega_df, 
              startRow = 104, startCol = 1)
     
    writeData(wb, "HC20107_sup", HC20107_RCH659_df, 
              startRow = 112, startCol = 1)
 
    writeData(wb, "HC20107_sup", HC20107_RCHNeo_df, 
              startRow = 173, startCol = 1)
    
     writeData(wb, "HC20107_sup", HC20107_Jadoo_df, 
              startRow = 186, startCol = 1)
     
    writeData(wb, "HC20107_sup", HC20107_Moksha_df, 
              startRow = 223, startCol = 1)             

    writeData(wb, "HC20107_sup", HC20107_Jangee_df, 
              startRow =257, startCol = 1)
    
     writeData(wb, "HC20107_sup", HC20107_MoneyMaker_df, 
              startRow = 294, startCol = 1)
     
    writeData(wb, "HC20107_sup", HC20107_ATM_df, 
              startRow = 312, startCol = 1)
              
              
              
    saveWorkbook(wb, "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH4_State_Hybrid_Rows.xlsx")
...
#B. Comining datasets datasets  for dashboard superiority rowwise 
    ...
    HC20107_Aatish_YLD <- data.frame(HC20107_Aatish_df)
    HC20107_Ajooba_YLD <- data.frame(HC20107_Ajooba_df)
    HC20107_Pinaka_YLD <- data.frame(HC20107_Pinaka_df)
    HC20107_MC5401_YLD <- data.frame(HC20107_MC5401_df)
    HC20107_Vega_YLD <- data.frame(HC20107_Vega_df)
    
    HC20107_RCH659_YLD <- data.frame(HC20107_RCH659_df)
    HC20107_RCHNeo_YLD <- data.frame(HC20107_RCHNeo_df)
    HC20107_Jadoo_YLD <- data.frame(HC20107_Jadoo_df)
    HC20107_Moksha_YLD <- data.frame(HC20107_Moksha_df)
    HC20107_Jangee_YLD <- data.frame(HC20107_Jangee_df)
    HC20107_MoneyMaker_YLD <- data.frame(HC20107_MoneyMaker_df)
    HC20107_ATM_YLD <- data.frame(HC20107_ATM_df)
    ...
    writeData(wb, "HC20107_sup", HC20107_Aatish_YLD, 
              startRow = 1, startCol = 1)
    
    writeData(wb, "HC20107_sup", HC20107_Ajooba_YLD, 
              startRow = 1, startCol = 14)
    
    writeData(wb, "HC20107_sup", HC20107_Pinaka_YLD, 
              startRow = 1, startCol = 28)
  
    
    writeData(wb, "HC20107_sup", HC20107_MC5401_YLD, 
              startRow = 1, startCol = 42)
    
    writeData(wb, "HC20107_sup", HC20107_Vega_YLD, 
              startRow = 1, startCol = 56)
    
    writeData(wb, "HC20107_sup", HC20107_RCH659_YLD, 
              startRow = 1, startCol = 70)
                  
    writeData(wb, "HC20107_sup", HC20107_RCHNeo_YLD, 
              startRow = 1, startCol = 84)
    
    writeData(wb, "HC20107_sup", HC20107_Jadoo_YLD, 
              startRow = 1, startCol = 98)
    
    writeData(wb, "HC20107_sup", HC20107_Moksha_YLD, 
              startRow = 1, startCol = 112)
                  
    writeData(wb, "HC20107_sup", HC20107_Jangee_YLD, 
              startRow = 1, startCol = 126)
    
    writeData(wb, "HC20107_sup", HC20107_MoneyMaker_YLD, 
              startRow = 1, startCol = 140)
    
    writeData(wb, "HC20107_sup",    HC20107_ATM_YLD , 
              startRow = 1, startCol = 154)
              

 saveWorkbook(wb, "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH5_State_Hybrid_Columns.xlsx")
...
# 21A. Genotype x Environment-Reression plots based Eberhar and Russel model HC20107 Vs Checks 
############################################################################################## 
```{r}    
#Dataset for HC20107 vs Checks 


HC20107_Yld <- cottonData%>%
    ...
    write_xlsx(list(GESummary =  HC20107_Yld),
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH6_GESummary.xlsx")
...
HC20107_Aatish_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Aatish) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Aatish)/Aatish*100,
           Superority  = round(Superority,2),Check = "Aatish")   
    

  
HC20107_Ajooba_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Ajooba) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Ajooba)/Ajooba*100,
           Superority  = round(Superority,2),Check = "Ajooba")   


  
HC20107_Pinaka_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Pinaka) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Pinaka)/Pinaka*100,
           Superority  = round(Superority,2),Check = "Pinaka")   



  
HC20107_MC5401_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,MC5401) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - MC5401)/MC5401*100,
           Superority  = round(Superority,2),Check = "MC5401")   


  
HC20107_Vega_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Vega) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Vega)/Vega*100,
           Superority  = round(Superority,2),Check = "Vega")   


HC20107_RCH659_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,RCH659) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - RCH659)/RCH659*100,
           Superority  = round(Superority,2),Check = "RCH659")   



  
HC20107_RCHNeo_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,RCHNeo) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - RCHNeo)/RCHNeo*100,
           Superority  = round(Superority,2),Check = "RCHNeo")   


HC20107_Jadoo_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Jadoo) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Jadoo)/Jadoo*100,
           Superority  = round(Superority,2),Check = "Jadoo")   


HC20107_Moksha_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Moksha) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Moksha)/Moksha*100,
           Superority  = round(Superority,2),Check = "Moksha")   


  
HC20107_Jangee_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,Jangee) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - Jangee)/Jangee*100,
           Superority  = round(Superority,2),Check = "Jangee")   


  
HC20107_MoneyMaker_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,MoneyMaker) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - MoneyMaker)/MoneyMaker*100,
           Superority  = round(Superority,2),Check = "MoneyMaker")   


HC20107_ATM_Reg <- HC20107_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20107,ATM) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20107 - ATM)/ATM*100,
...
#Data set for Aatish
    ...
    HC20107_Aatish_CC <- HC20107_Aatish_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsAatish",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Aatish)
    
    HC20107_Aatish_NH <- HC20107_Aatish_Reg %>%
    select(-Superority, -Aatish,-Check) %>%
    mutate(CheckType = "HC20107VsAatish",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Aatish_CC)
...
#Data set for Ajooba
    ...
    HC20107_Ajooba_CC <- HC20107_Ajooba_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsAjooba",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Ajooba)
    
    HC20107_Ajooba_NH <- HC20107_Ajooba_Reg %>%
    select(-Superority, -Ajooba,-Check) %>%
    mutate(CheckType = "HC20107VsAjooba",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Ajooba_CC)


#Data set for Pinaka
    
    HC20107_Pinaka_CC <- HC20107_Pinaka_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsPinaka",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Pinaka)
    
    HC20107_Pinaka_NH <- HC20107_Pinaka_Reg %>%
    select(-Superority, -Pinaka,-Check) %>%
    mutate(CheckType = "HC20107VsPinaka",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Pinaka_CC)



#Data set for MC5401
    HC20107_MC5401_CC <- HC20107_MC5401_Reg%>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsMC5401",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = MC5401)
    
    HC20107_MC5401_NH <- HC20107_MC5401_Reg %>%
    select(-Superority, -MC5401,-Check) %>%
    mutate(CheckType = "HC20107VsMC5401",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_MC5401_CC)

    

#Data set for Vega
    HC20107_Vega_CC <- HC20107_Vega_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsVega",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Vega)
    
    HC20107_Vega_NH <- HC20107_Vega_Reg %>%
    select(-Superority, -Vega,-Check) %>%
    mutate(CheckType = "HC20107VsVega",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Vega_CC)
    
   
#Data set for RCH659
    HC20107_RCH659_CC <- HC20107_RCH659_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsRCH659",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = RCH659)
    HC20107_RCH659_NH <- HC20107_RCH659_Reg %>%
    select(-Superority, -RCH659,-Check) %>%
    mutate(CheckType = "HC20107VsRCH659",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_RCH659_CC)


#Data set for RCHNeo
    HC20107_RCHNeo_CC <- HC20107_RCHNeo_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsRCHNeo",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = RCHNeo)
    
    HC20107_RCHNeo_NH <- HC20107_RCHNeo_Reg %>%
    select(-Superority, -RCHNeo,-Check) %>%
    mutate(CheckType = "HC20107VsRCHNeo",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_RCHNeo_CC)


#Data set for Jadoo
    HC20107_Jadoo_CC <- HC20107_Jadoo_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsJadoo",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Jadoo)
    HC20107_Jadoo_NH <- HC20107_Jadoo_Reg %>%
    select(-Superority, -Jadoo,-Check) %>%
    mutate(CheckType = "HC20107VsJadoo",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Jadoo_CC)

#Data set for Moksha
    HC20107_Moksha_CC <- HC20107_Moksha_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsMoksha",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Moksha)
    
    HC20107_Moksha_NH <- HC20107_Moksha_Reg %>%
    select(-Superority, -Moksha,-Check) %>%
    mutate(CheckType = "HC20107VsMoksha",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Moksha_CC)
    

#Data set for Jangee
    HC20107_Jangee_CC <- HC20107_Jangee_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsJangee",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Jangee)
    
    HC20107_Jangee_NH <- HC20107_Jangee_Reg %>%
    select(-Superority, -Jangee,-Check) %>%
    mutate(CheckType = "HC20107VsJangee",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_Jangee_CC)


#Data set for MoneyMaker
    HC20107_MoneyMaker_CC <- HC20107_MoneyMaker_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsMoneyMaker",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = MoneyMaker)
    
    HC20107_MoneyMaker_NH <- HC20107_MoneyMaker_Reg %>%
    select(-Superority, -MoneyMaker,-Check) %>%
    mutate(CheckType = "HC20107VsMoneyMaker",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_MoneyMaker_CC)
 
 
#Data set for ATM
    HC20107_ATM_CC <- HC20107_ATM_Reg %>%
    select(-Superority, -HC20107,-Check) %>%
    mutate(CheckType = "HC20107VsATM",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = ATM)
    
    HC20107_ATM_NH <- HC20107_ATM_Reg %>%
    select(-Superority, -ATM,-Check) %>%
    mutate(CheckType = "HC20107VsATM",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20107) %>%
    rbind(HC20107_ATM_CC)
    
#Comibing all the datsets created for linear regression
    HC20107_Reg <- rbind(HC20107_Aatish_NH,
    HC20107_Ajooba_NH,
    HC20107_Pinaka_NH,
    HC20107_MC5401_NH,
    HC20107_Vega_NH,
    ...
    HC20107_Moksha_NH,
    HC20107_Jangee_NH,
    HC20107_MoneyMaker_NH,
    HC20107_ATM_NH)
    
write_xlsx(list(Regresssion_data =   HC20107_Reg),
    "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH7_HC20107_Reg.xlsx") 
      
      
```

 
# 21B. Reression plots based Eberhar and Russel model HC20107 Vs Checks 

############################################################################################## 
```{r}
          
t7 <- "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/HC20107_Loc_Hybrid_Reg.pptx"

    my.formula <- y ~ x
    HC20107_Loc_Hybrid_Reg <- ggplot(data =  HC20107_Reg , 
                                 aes(x =  EnvIndex,
                                     y = Yield,
                                     color = Group)) +
    ...
    fig_vg7 <- dml(ggobj =   HC20107_Loc_Hybrid_Reg )
...
```{r}    
 #create Barplots exhibiting Mean Yield  and Winning percentage of Hybrids
     ...
     HC20107_barplot <-  StateHybridMeans  %>% 
      ...
      mutate(WinsPCT = (ifelse(Hybrid == "HC20107",paste(Wins_PCT, "%", Sep = ""), 
      Wins_PCT ))) 
    
    
    t8 <- "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/HC20107_winsPcT.pptx"
    
    HC20107_winsPcTplot <- ggplot(data =  HC20107_barplot ,
                                  aes(x = State, y = Yield,
      ...
      labs( title = "Winning Percentage of HC20107 over the Checks across the States ",
            x = "Sates",
            y = "Yiled(kg/acre)" ) +
    ...
    fig_vg8 <- dml(ggobj =   HC20107_winsPcTplot)
    read_pptx() %>% 
      add_slide(layout = "Title Only", master = "Office Theme") %>% 
...
# 23. Barplots exhibiting : Early  vs Totsl Yield for  HC20107 Vs Checks  
##############################################################################################
...
#Standability score for HC20107 and Checks
    ...
    Standability_Score <- pest_disease_data   %>% 
     ...
     select(Standability,
            HC20107_Aatish_HC20107_NH,
            HC20107_Aatish_Aatish_CC, 
            HC20107_Ajooba_HC20107_NH,
            HC20107_Ajooba_Ajooba_CC,
            HC20107_Pinaka_HC20107_NH,
            HC20107_Pinaka_Pinaka_CC,
            HC20107_MC5401_HC20107_NH,
            HC20107_MC5401_MC5401_CC,
            HC20107_Vega_HC20107_NH,
            HC20107_Vega_Vega_CC,
            HC20107_RCH659_HC20107_NH,
            HC20107_RCH659_RCH659_CC,
            HC20107_RCHNeo_HC20107_NH,
            HC20107_RCHNeo_RCHNeo_CC,
            HC20107_Jadoo_HC20107_NH,
            HC20107_Jadoo_Jadoo_CC,
            HC20107_Moksha_HC20107_NH,
            HC20107_Moksha_Moksha_CC,
            HC20107_Jangee_HC20107_NH,
            HC20107_Jangee_Jangee_CC,
            HC20107_MoneyMaker_HC20107_NH,
            HC20107_MoneyMaker_MoneyMaker_CC,
            HC20107_ATM_HC20107_NH,
            HC20107_ATM_ATM_CC)
...
Standability_Loc <- pest_disease_data   %>%
      ...
      filter(HC20107 %in% c("3", "4", "5" )|
             ...
             ATM %in% c("3", "4", "5" )) %>% 
      select(Trial,State, District,Market,Village,HC20107,Aatish,Ajooba, Pinaka, MC5401, Vega,
    ...
    write_xlsx(list(Standability_Score  =   Standability_Score ,
      ...
      "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH10_HC20107_Standability.xlsx")
 

    
# Pickabilty score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list( Pickabilty_Score  =   Pickabilty_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH11_HC20107_Pickabilty.xlsx")
    
    
    
#Stembending score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Stembending_Score =   Stembending_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH12_HC20107_Stembending.xlsx")
    

#Suckingpest_60days score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Suckingpest_60days_Score  =   Suckingpest_60days_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH13_HC20107_Suckingpest_60days.xlsx")
    
    
    
    
#Suckingpest_120days score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Suckingpest_120days  =   Suckingpest_120days_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH14_HC20107_Suckingpest_120days.xlsx")
    
    
    
#Bollworm score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list( Bollworm  =   Bollworm_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH15_HC20107_Bollworm.xlsx")
    
    

    
#Leafcurl score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Leafcurl_Score  =   Leafcurl_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH16_HC20107_Leafcurl.xlsx")
    
    
    
#Wiltscore score for HC20107 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(StalkLodgScore  =   Wiltscore_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20107/NH17_HC20107_Wiltscore.xlsx")
```   
...
``` {r}
    ...
    agron_quality_diseaes_HC20107 <- cbind(HybridMeans_radar,  
...
group <- c("HC20107_Aatish", "Aatish",
           "HC20107_Ajooba", "Ajooba",
           "HC20107_ATM", "ATM",
           "HC20107_Jadoo", "Jadoo",
           "HC20107_Jangee", "Jangee",
           "HC20107_MC5401", "MC5401",           
           "HC20107_Moksha", "Moksha",
           "HC20107_MoneyMaker", "MoneyMaker",
           "HC20107_Pinaka", "Pinaka",
           "HC20107_RCH659", "RCH659",
           "HC20107_RCHNeo", "RCHNeo",
           "HC20107_Vega", "Vega")

# Assigning  the the group varaible  to agron_quality_diseaes_HT20404

agron_quality_diseaes_HC20107$Hybrid <- group

# Deleting  the variables, hyb_group and Wins

radartest <- agron_quality_diseaes_HC20107 %>% 
  select(-Hybrid_group) %>% 
  dplyr::rename(group = Hybrid)



radar_HC20107plot <- ggradar2(radartest,
                              grid.label.size = 4,
                              axis.label.size = 4, 
                              group.point.size = 5,
                              group.line.width = 1.5,
                              legend.text.size= 10) +
  labs(title = "Radar Chart comparing HC20107 with Checks") 



#Multiple plots by subgroups

facet1 <- agron_quality_diseaes_HC20107$Hybrid_group        
radartest2 <- cbind(radartest,facet1) 

radar_HC20107plot1 <- ggradar2(  radartest2 ,
                                 multiplots = TRUE,
                                 grid.label.size = 3,
                                 axis.label.size = 2, 
                                 group.point.size = 5,
