...
setwd("C:/Users/Naidu/Desktop/R_Projects/Investment/Rprogram")
...
# a package to handling the messy data and organize it
    install.packages("tidyverse")

# The package to download a spreadsheet from a website
    install.packages("RCurl") 

# This package performs time-series applications
   install.packages("forecast")

# A package for analyze financial/ time-series data
   install.packages("PerformanceAnalytics") 
# package for read_delim() function
...
```{r}
#select Varibales  for Head to Head analysis HC20103_Aatish

HC20103_Aatish <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Aatish")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Aatish", 
         Check_group = "Aatish", 
...
#select Varibales  for Head to Head analysis HC20103_Ajooba

HC20103_Ajooba <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Ajooba")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Ajooba", 
         Check_group = "Ajooba", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_MC5401

HC20103_MC5401 <- cottonData%>%
  filter(Hybrid %in% c("HC20103","MC5401")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_MC5401", 
         Check_group = "MC5401", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_Pinaka

HC20103_Pinaka <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Pinaka")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Pinaka", 
         Check_group = "Pinaka", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_Vega

HC20103_Vega <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Vega")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Vega", 
         Check_group = "Vega", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_RCH659

HC20103_RCH659 <- cottonData%>%
  filter(Hybrid %in% c("HC20103","RCH659")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_RCH659", 
         Check_group = "RCH659", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_Jadoo

HC20103_Jadoo <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Jadoo")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Jadoo", 
         Check_group = "Jadoo", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_Moksha

HC20103_Moksha <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Moksha")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Moksha", 
         Check_group = "Moksha", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_Jangee

HC20103_Jangee <- cottonData%>%
  filter(Hybrid %in% c("HC20103","Jangee")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_Jangee", 
         Check_group = "Jangee", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_MoneyMaker

HC20103_MoneyMaker <- cottonData%>%
  filter(Hybrid %in% c("HC20103","MoneyMaker")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_MoneyMaker", 
...
#select Varibales  for Head to Head analysis HC20103_ATM

HC20103_ATM <- cottonData%>%
  filter(Hybrid %in% c("HC20103","ATM")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_ATM", 
         Check_group = "ATM", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
#select Varibales  for Head to Head analysis HC20103_RCHNeo

HC20103_RCHNeo <- cottonData%>%
  filter(Hybrid %in% c("HC20103","RCHNeo")) %>% 
  group_by(Trial) %>%
  mutate(Number = n()) %>% 
  filter(Number > 1) %>% 
  mutate(Hybrid_group = "HC20103_RCHNeo", 
         Check_group = "RCHNeo", 
         Difference = (Yield - lag(Yield, 
                                           default = Yield[-1])), 
         Win = (ifelse(Hybtype == "NH" & Difference >= 0,
...
```{r}
All_combined <- bind_rows(HC20103_Aatish,
                          HC20103_Ajooba,
                          HC20103_Pinaka,
                          HC20103_MC5401,
                          HC20103_Vega,
...
"D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH1_Means_Wins_Yield_Superiority.xlsx")
...
superority <- HybridMeans %>% 
  ...
  mutate(Aatish_Superiority	=(HC20103_AatishNH-HC20103_AatishCC)/HC20103_AatishCC*100,
      Ajooba_Superiority =(HC20103_AjoobaNH-HC20103_AjoobaCC)/HC20103_AjoobaCC*100,
      MC5401_Superiority =(HC20103_MC5401NH-HC20103_MC5401CC)/HC20103_MC5401CC*100,
      Pinaka_Superiority =(HC20103_PinakaNH-HC20103_PinakaCC)/HC20103_PinakaCC*100,
      Vega_Superiority	= (HC20103_VegaNH-HC20103_VegaCC)/HC20103_VegaCC*100,
      ATM_Superiority	= (HC20103_ATMNH-HC20103_ATMCC)/HC20103_ATMCC*100,
      Jadoo_Superiority	= (HC20103_JadooNH-HC20103_JadooCC)/HC20103_JadooCC*100,
      Jangee_Superiority =(HC20103_JangeeNH-HC20103_JangeeCC)/HC20103_JangeeCC*100,
      Moksha_Superiority =(HC20103_MokshaNH-HC20103_MokshaCC)/HC20103_MokshaCC*100,
    MoneyMaker_Superiority = 
    (HC20103_MoneyMakerNH-HC20103_MoneyMakerCC)/HC20103_MoneyMakerCC*100,
    RCH659_Superiority =(HC20103_RCH659NH-HC20103_RCH659CC)/HC20103_RCH659CC*100,
    RCHNeo_Superiority	=(HC20103_RCHNeoNH-HC20103_RCHNeoCC)/HC20103_RCHNeoCC*100) %>%   mutate(across(is.numeric, ~ round(., 2))) %>% 
  mutate_all(funs(replace(., .== "Inf", NA))) %>%   
  mutate_all(funs(replace(., .== 0.00, NA))) %>%  
            rename("Attribute" = variable) %>% 
      select(Attribute, 
              HC20103_AatishNH, HC20103_AatishCC, Aatish_Superiority,
              HC20103_AjoobaNH, HC20103_AjoobaCC, Ajooba_Superiority,
              HC20103_PinakaNH, HC20103_PinakaCC, Pinaka_Superiority,
              HC20103_MC5401NH, HC20103_MC5401CC, MC5401_Superiority,
              HC20103_VegaNH,     HC20103_VegaCC,  Vega_Superiority, 
              HC20103_RCH659NH, HC20103_RCH659CC, RCH659_Superiority,
              HC20103_JadooNH,  HC20103_JadooCC, Jadoo_Superiority,
              HC20103_JangeeNH, HC20103_JangeeCC, Jangee_Superiority,
              HC20103_MokshaNH, HC20103_MokshaCC,Moksha_Superiority,
              HC20103_MoneyMakerNH, HC20103_MoneyMakerCC,MoneyMaker_Superiority,
              HC20103_ATMNH, HC20103_ATMCC,ATM_Superiority,
              HC20103_RCHNeoNH, HC20103_RCHNeoCC,RCHNeo_Superiority,
              everything()) %>% 
...
"D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH2_Superiority.xlsx")
...
```{r}
HC20103_all_Hybrid <- All_combined %>% 
    ungroup() %>% 
...
HC20103_all_State <- All_combined %>% 
    ungroup() %>% 
...
# Combine HC20103_all_State  and HC20103_all_Hybrid by rows

HC20103_State_Trials_Wins <- bind_rows(HC20103_all_State,
                                       HC20103_all_Hybrid)


write_xlsx(list(HC20103_All = HC20103_State_Trials_Wins),
           "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH3_State_Trials_Means_Wins_B.xlsx")
...
# 20A. Creating Summary tables for Excel Dash boards for HC20103
...
```{r}
HC20103_data <- All_combined %>% 
    select(Trial, State, Loc, Cropmanagmt,Sowingdate,Cultivation, Hybrid, Check_group, Yield) %>% 
      filter(Hybrid == "HC20103")
HC20103_checks <- All_combined %>% 
    filter(Hybrid != "HC20103") %>% 
    select(Trial, State, Yield)  

HC20103_checks_df <- cbind(HC20103_data,
                               HC20103_checks) %>% 
...
#Create  Datasets  for HEad to Head comparison of HC20103 with all Checks 


HC20103_Aatish_df <- HC20103_checks_df %>% 
  ...
  rename(HC20103 = NH_Yield,
         Aatish = CC_Yield)



HC20103_Ajooba_df <- HC20103_checks_df %>% 
...
HC20103_Pinaka_df <- HC20103_checks_df %>% 
...
HC20103_MC5401_df <- HC20103_checks_df %>% 
...
HC20103_Vega_df <- HC20103_checks_df %>% 
...
HC20103_RCH659_df <- HC20103_checks_df %>% 
...
HC20103_RCHNeo_df <- HC20103_checks_df %>% 
...
HC20103_Jadoo_df <- HC20103_checks_df %>% 
...
HC20103_Moksha_df <- HC20103_checks_df %>% 
...
HC20103_Jangee_df <- HC20103_checks_df %>% 
...
HC20103_MoneyMaker_df <- HC20103_checks_df %>% 
...
HC20103_ATM_df <- HC20103_checks_df %>% 
...
# A. Workbook for checkwise superority     
  ...
  wb <- createWorkbook()
    ...
    addWorksheet(wb, "HC20103_sup")
    
    writeData(wb, "HC20103_sup", HC20103_Aatish_df, 
              startRow = 1, startCol = 1)
    
     writeData(wb, "HC20103_sup", HC20103_Ajooba_df, 
              startRow = 47, startCol = 1)
     
    writeData(wb, "HC20103_sup", HC20103_Pinaka_df, 
              startRow = 63, startCol = 1)     
    
    writeData(wb, "HC20103_sup", HC20103_MC5401_df, 
              startRow = 92, startCol = 1)
    
    writeData(wb, "HC20103_sup", HC20103_Vega_df, 
              startRow = 104, startCol = 1)
     
    writeData(wb, "HC20103_sup", HC20103_RCH659_df, 
              startRow = 112, startCol = 1)
 
    writeData(wb, "HC20103_sup", HC20103_RCHNeo_df, 
              startRow = 173, startCol = 1)
    
     writeData(wb, "HC20103_sup", HC20103_Jadoo_df, 
              startRow = 186, startCol = 1)
     
    writeData(wb, "HC20103_sup", HC20103_Moksha_df, 
              startRow = 223, startCol = 1)             

    writeData(wb, "HC20103_sup", HC20103_Jangee_df, 
              startRow =257, startCol = 1)
    
     writeData(wb, "HC20103_sup", HC20103_MoneyMaker_df, 
              startRow = 294, startCol = 1)
     
    writeData(wb, "HC20103_sup", HC20103_ATM_df, 
              startRow = 312, startCol = 1)
              
              
              
    saveWorkbook(wb, "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH4_State_Hybrid_Rows.xlsx")
...
#B. Comining datasets datasets  for dashboard superiority rowwise 
    ...
    HC20103_Aatish_YLD <- data.frame(HC20103_Aatish_df)
    HC20103_Ajooba_YLD <- data.frame(HC20103_Ajooba_df)
    HC20103_Pinaka_YLD <- data.frame(HC20103_Pinaka_df)
    HC20103_MC5401_YLD <- data.frame(HC20103_MC5401_df)
    HC20103_Vega_YLD <- data.frame(HC20103_Vega_df)
    
    HC20103_RCH659_YLD <- data.frame(HC20103_RCH659_df)
    HC20103_RCHNeo_YLD <- data.frame(HC20103_RCHNeo_df)
    HC20103_Jadoo_YLD <- data.frame(HC20103_Jadoo_df)
    HC20103_Moksha_YLD <- data.frame(HC20103_Moksha_df)
    HC20103_Jangee_YLD <- data.frame(HC20103_Jangee_df)
    HC20103_MoneyMaker_YLD <- data.frame(HC20103_MoneyMaker_df)
    HC20103_ATM_YLD <- data.frame(HC20103_ATM_df)
    ...
    writeData(wb, "HC20103_sup", HC20103_Aatish_YLD, 
              startRow = 1, startCol = 1)
    
    writeData(wb, "HC20103_sup", HC20103_Ajooba_YLD, 
              startRow = 1, startCol = 14)
    
    writeData(wb, "HC20103_sup", HC20103_Pinaka_YLD, 
              startRow = 1, startCol = 28)
  
    
    writeData(wb, "HC20103_sup", HC20103_MC5401_YLD, 
              startRow = 1, startCol = 42)
    
    writeData(wb, "HC20103_sup", HC20103_Vega_YLD, 
              startRow = 1, startCol = 56)
    
    writeData(wb, "HC20103_sup", HC20103_RCH659_YLD, 
              startRow = 1, startCol = 70)
                  
    writeData(wb, "HC20103_sup", HC20103_RCHNeo_YLD, 
              startRow = 1, startCol = 84)
    
    writeData(wb, "HC20103_sup", HC20103_Jadoo_YLD, 
              startRow = 1, startCol = 98)
    
    writeData(wb, "HC20103_sup", HC20103_Moksha_YLD, 
              startRow = 1, startCol = 112)
                  
    writeData(wb, "HC20103_sup", HC20103_Jangee_YLD, 
              startRow = 1, startCol = 126)
    
    writeData(wb, "HC20103_sup", HC20103_MoneyMaker_YLD, 
              startRow = 1, startCol = 140)
    
    writeData(wb, "HC20103_sup",    HC20103_ATM_YLD , 
              startRow = 1, startCol = 154)
              

 saveWorkbook(wb, "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH5_State_Hybrid_Columns.xlsx")
...
# 21A. Genotype x Environment-Reression plots based Eberhar and Russel model HC20103 Vs Checks 
############################################################################################## 
```{r}    
#Dataset for HC20103 vs Checks 


HC20103_Yld <- cottonData%>%
    ...
    write_xlsx(list(GESummary =  HC20103_Yld),
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH6_GESummary.xlsx")
...
HC20103_Aatish_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Aatish) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Aatish)/Aatish*100,
           Superority  = round(Superority,2),Check = "Aatish")   
    

  
HC20103_Ajooba_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Ajooba) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Ajooba)/Ajooba*100,
           Superority  = round(Superority,2),Check = "Ajooba")   


  
HC20103_Pinaka_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Pinaka) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Pinaka)/Pinaka*100,
           Superority  = round(Superority,2),Check = "Pinaka")   



  
HC20103_MC5401_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,MC5401) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - MC5401)/MC5401*100,
           Superority  = round(Superority,2),Check = "MC5401")   


  
HC20103_Vega_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Vega) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Vega)/Vega*100,
           Superority  = round(Superority,2),Check = "Vega")   


HC20103_RCH659_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,RCH659) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - RCH659)/RCH659*100,
           Superority  = round(Superority,2),Check = "RCH659")   



  
HC20103_RCHNeo_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,RCHNeo) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - RCHNeo)/RCHNeo*100,
           Superority  = round(Superority,2),Check = "RCHNeo")   


HC20103_Jadoo_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Jadoo) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Jadoo)/Jadoo*100,
           Superority  = round(Superority,2),Check = "Jadoo")   


HC20103_Moksha_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Moksha) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Moksha)/Moksha*100,
           Superority  = round(Superority,2),Check = "Moksha")   


  
HC20103_Jangee_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,Jangee) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - Jangee)/Jangee*100,
           Superority  = round(Superority,2),Check = "Jangee")   


  
HC20103_MoneyMaker_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,MoneyMaker) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - MoneyMaker)/MoneyMaker*100,
           Superority  = round(Superority,2),Check = "MoneyMaker")   


HC20103_ATM_Reg <- HC20103_Yld %>% 
    select(Trial,State,Loc,Cropmanagmt,Cultivation,Sowingdate, State, Locationmean_YLD, 
    HC20103,ATM) %>% 
    drop_na() %>% 
    mutate(Superority = (HC20103 - ATM)/ATM*100,
...
#Data set for Aatish
    ...
    HC20103_Aatish_CC <- HC20103_Aatish_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsAatish",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Aatish)
    
    HC20103_Aatish_NH <- HC20103_Aatish_Reg %>%
    select(-Superority, -Aatish,-Check) %>%
    mutate(CheckType = "HC20103VsAatish",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Aatish_CC)
...
#Data set for Ajooba
    ...
    HC20103_Ajooba_CC <- HC20103_Ajooba_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsAjooba",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Ajooba)
    
    HC20103_Ajooba_NH <- HC20103_Ajooba_Reg %>%
    select(-Superority, -Ajooba,-Check) %>%
    mutate(CheckType = "HC20103VsAjooba",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Ajooba_CC)


#Data set for Pinaka
    
    HC20103_Pinaka_CC <- HC20103_Pinaka_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsPinaka",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Pinaka)
    
    HC20103_Pinaka_NH <- HC20103_Pinaka_Reg %>%
    select(-Superority, -Pinaka,-Check) %>%
    mutate(CheckType = "HC20103VsPinaka",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Pinaka_CC)



#Data set for MC5401
    HC20103_MC5401_CC <- HC20103_MC5401_Reg%>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsMC5401",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = MC5401)
    
    HC20103_MC5401_NH <- HC20103_MC5401_Reg %>%
    select(-Superority, -MC5401,-Check) %>%
    mutate(CheckType = "HC20103VsMC5401",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_MC5401_CC)

    

#Data set for Vega
    HC20103_Vega_CC <- HC20103_Vega_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsVega",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Vega)
    
    HC20103_Vega_NH <- HC20103_Vega_Reg %>%
    select(-Superority, -Vega,-Check) %>%
    mutate(CheckType = "HC20103VsVega",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Vega_CC)
    
   
#Data set for RCH659
    HC20103_RCH659_CC <- HC20103_RCH659_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsRCH659",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = RCH659)
    HC20103_RCH659_NH <- HC20103_RCH659_Reg %>%
    select(-Superority, -RCH659,-Check) %>%
    mutate(CheckType = "HC20103VsRCH659",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_RCH659_CC)


#Data set for RCHNeo
    HC20103_RCHNeo_CC <- HC20103_RCHNeo_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsRCHNeo",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = RCHNeo)
    
    HC20103_RCHNeo_NH <- HC20103_RCHNeo_Reg %>%
    select(-Superority, -RCHNeo,-Check) %>%
    mutate(CheckType = "HC20103VsRCHNeo",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_RCHNeo_CC)


#Data set for Jadoo
    HC20103_Jadoo_CC <- HC20103_Jadoo_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsJadoo",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Jadoo)
    HC20103_Jadoo_NH <- HC20103_Jadoo_Reg %>%
    select(-Superority, -Jadoo,-Check) %>%
    mutate(CheckType = "HC20103VsJadoo",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Jadoo_CC)

#Data set for Moksha
    HC20103_Moksha_CC <- HC20103_Moksha_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsMoksha",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Moksha)
    
    HC20103_Moksha_NH <- HC20103_Moksha_Reg %>%
    select(-Superority, -Moksha,-Check) %>%
    mutate(CheckType = "HC20103VsMoksha",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Moksha_CC)
    

#Data set for Jangee
    HC20103_Jangee_CC <- HC20103_Jangee_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsJangee",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = Jangee)
    
    HC20103_Jangee_NH <- HC20103_Jangee_Reg %>%
    select(-Superority, -Jangee,-Check) %>%
    mutate(CheckType = "HC20103VsJangee",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_Jangee_CC)


#Data set for MoneyMaker
    HC20103_MoneyMaker_CC <- HC20103_MoneyMaker_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsMoneyMaker",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = MoneyMaker)
    
    HC20103_MoneyMaker_NH <- HC20103_MoneyMaker_Reg %>%
    select(-Superority, -MoneyMaker,-Check) %>%
    mutate(CheckType = "HC20103VsMoneyMaker",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_MoneyMaker_CC)
 
 
#Data set for ATM
    HC20103_ATM_CC <- HC20103_ATM_Reg %>%
    select(-Superority, -HC20103,-Check) %>%
    mutate(CheckType = "HC20103VsATM",
    Group     = "Check" ) %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = ATM)
    
    HC20103_ATM_NH <- HC20103_ATM_Reg %>%
    select(-Superority, -ATM,-Check) %>%
    mutate(CheckType = "HC20103VsATM",
    Group = "New Hybrid") %>%
    rename(EnvIndex = Locationmean_YLD,
    Yield = HC20103) %>%
    rbind(HC20103_ATM_CC)
    
#Comibing all the datsets created for linear regression
    HC20103_Reg <- rbind(HC20103_Aatish_NH,
    HC20103_Ajooba_NH,
    HC20103_Pinaka_NH,
    HC20103_MC5401_NH,
    HC20103_Vega_NH,
    ...
    HC20103_Moksha_NH,
    HC20103_Jangee_NH,
    HC20103_MoneyMaker_NH,
    HC20103_ATM_NH)
    
write_xlsx(list(Regresssion_data =   HC20103_Reg),
    "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH7_HC20103_Reg.xlsx") 
      
      
```

 
# 21B. Reression plots based Eberhar and Russel model HC20103 Vs Checks 

############################################################################################## 
```{r}
          
t7 <- "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/HC20103_Loc_Hybrid_Reg.pptx"

    my.formula <- y ~ x
    HC20103_Loc_Hybrid_Reg <- ggplot(data =  HC20103_Reg , 
                                 aes(x =  EnvIndex,
                                     y = Yield,
                                     color = Group)) +
    ...
    fig_vg7 <- dml(ggobj =   HC20103_Loc_Hybrid_Reg )
...
```{r}    
 #create Barplots exhibiting Mean Yield  and Winning percentage of Hybrids
     ...
     HC20103_barplot <-  StateHybridMeans  %>% 
      ...
      mutate(WinsPCT = (ifelse(Hybrid == "HC20103",paste(Wins_PCT, "%", Sep = ""), 
      Wins_PCT ))) 
    
    
    t8 <- "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/HC20103_winsPcT.pptx"
    
    HC20103_winsPcTplot <- ggplot(data =  HC20103_barplot ,
                                  aes(x = State, y = Yield,
      ...
      labs( title = "Winning Percentage of HC20103 over the Checks across the States ",
            x = "Sates",
            y = "Yiled(kg/acre)" ) +
    ...
    fig_vg8 <- dml(ggobj =   HC20103_winsPcTplot)
    read_pptx() %>% 
      add_slide(layout = "Title Only", master = "Office Theme") %>% 
...
# 23. Barplots exhibiting : Early  vs Totsl Yield for  HC20103 Vs Checks  
##############################################################################################
...
#Standability score for HC20103 and Checks
    ...
    Standability_Score <- pest_disease_data   %>% 
     ...
     select(Standability,
            HC20103_Aatish_HC20103_NH,
            HC20103_Aatish_Aatish_CC, 
            HC20103_Ajooba_HC20103_NH,
            HC20103_Ajooba_Ajooba_CC,
            HC20103_Pinaka_HC20103_NH,
            HC20103_Pinaka_Pinaka_CC,
            HC20103_MC5401_HC20103_NH,
            HC20103_MC5401_MC5401_CC,
            HC20103_Vega_HC20103_NH,
            HC20103_Vega_Vega_CC,
            HC20103_RCH659_HC20103_NH,
            HC20103_RCH659_RCH659_CC,
            HC20103_RCHNeo_HC20103_NH,
            HC20103_RCHNeo_RCHNeo_CC,
            HC20103_Jadoo_HC20103_NH,
            HC20103_Jadoo_Jadoo_CC,
            HC20103_Moksha_HC20103_NH,
            HC20103_Moksha_Moksha_CC,
            HC20103_Jangee_HC20103_NH,
            HC20103_Jangee_Jangee_CC,
            HC20103_MoneyMaker_HC20103_NH,
            HC20103_MoneyMaker_MoneyMaker_CC,
            HC20103_ATM_HC20103_NH,
            HC20103_ATM_ATM_CC)
...
Standability_Loc <- pest_disease_data   %>%
      ...
      filter(HC20103 %in% c("3", "4", "5" )|
             ...
             ATM %in% c("3", "4", "5" )) %>% 
      select(Trial,State, District,Market,Village,HC20103,Aatish,Ajooba, Pinaka, MC5401, Vega,
    ...
    write_xlsx(list(Standability_Score  =   Standability_Score ,
      ...
      "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH10_HC20103_Standability.xlsx")
 

    
# Pickabilty score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list( Pickabilty_Score  =   Pickabilty_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH11_HC20103_Pickabilty.xlsx")
    
    
    
#Stembending score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Stembending_Score =   Stembending_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH12_HC20103_Stembending.xlsx")
    

#Suckingpest_60days score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Suckingpest_60days_Score  =   Suckingpest_60days_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH13_HC20103_Suckingpest_60days.xlsx")
    
    
    
    
#Suckingpest_120days score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Suckingpest_120days  =   Suckingpest_120days_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH14_HC20103_Suckingpest_120days.xlsx")
    
    
    
#Bollworm score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list( Bollworm  =   Bollworm_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH15_HC20103_Bollworm.xlsx")
    
    

    
#Leafcurl score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(Leafcurl_Score  =   Leafcurl_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH16_HC20103_Leafcurl.xlsx")
    
    
    
#Wiltscore score for HC20103 and Checks
...
# Locations  where  severe StalkLodg score (3,4,5 scores)
    ...
    write_xlsx(list(StalkLodgScore  =   Wiltscore_Score ,
               ...
               "D:/ProductDevelopment/R_Projects/Cotton/cotton_SC/HAT/Results/HC20103/NH17_HC20103_Wiltscore.xlsx")
```   
...
``` {r}
    ...
    agron_quality_diseaes_HC20103 <- cbind(HybridMeans_radar,  
...
group <- c("HC20103_Aatish", "Aatish",
           "HC20103_Ajooba", "Ajooba",
           "HC20103_ATM", "ATM",
           "HC20103_Jadoo", "Jadoo",
           "HC20103_Jangee", "Jangee",
           "HC20103_MC5401", "MC5401",           
           "HC20103_Moksha", "Moksha",
           "HC20103_MoneyMaker", "MoneyMaker",
           "HC20103_Pinaka", "Pinaka",
           "HC20103_RCH659", "RCH659",
           "HC20103_RCHNeo", "RCHNeo",
           "HC20103_Vega", "Vega")

# Assigning  the the group varaible  to agron_quality_diseaes_HT20404

agron_quality_diseaes_HC20103$Hybrid <- group

# Deleting  the variables, hyb_group and Wins

radartest <- agron_quality_diseaes_HC20103 %>% 
  select(-Hybrid_group) %>% 
  dplyr::rename(group = Hybrid)



radar_HC20103plot <- ggradar2(radartest,
                              grid.label.size = 4,
                              axis.label.size = 4, 
                              group.point.size = 5,
                              group.line.width = 1.5,
                              legend.text.size= 10) +
  labs(title = "Radar Chart comparing HC20103 with Checks") 



#Multiple plots by subgroups

facet1 <- agron_quality_diseaes_HC20103$Hybrid_group        
radartest2 <- cbind(radartest,facet1) 

radar_HC20103plot1 <- ggradar2(  radartest2 ,
                                 multiplots = TRUE,
                                 grid.label.size = 3,
                                 axis.label.size = 2, 
                                 group.point.size = 5,
